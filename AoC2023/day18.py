...
def pt1(input):
    ...
    board = board[len(board)//2+ int(len(board)%2):len(board)]+board[0:len(board)//2]
    floodfill(board)
    #print(board)
...
def pt2(input):
    ...
    print("setting up the map")
    maxx = getmaxsize(newinput)[0]//4
    maxy = getmaxsize(newinput)[1]//4
    print(f"{maxx}, {maxy}")
    # TODO change to bitmap for storage 
    board = [["." for x in range(maxx +1)] for i in range(maxy+1)]
    pos = (0,0)
